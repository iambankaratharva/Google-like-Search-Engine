...
public class KVSClient implements KVS {
	...
	public void putBatch(String tableName, Iterable<FlamePair> flamePair) throws IOException {
		if (!haveWorkers)
			downloadWorkers();
		Map<String, List<Row>> workerToRowMap = new HashMap<>();

		for (FlamePair pair : flamePair) {
			String rowKey = pair._1();
			if (rowKey.contains(" ")) {
				continue;
			}
			String kvsWorkerAddress = workers.elementAt(workerIndexForKey(rowKey)).address;
			String column = Hasher.hash(UUID.randomUUID().toString().replace("-", ""));

			List<Row> rowsOfCurrentWorker = workerToRowMap.computeIfAbsent(kvsWorkerAddress, k -> new ArrayList<>());

			Row row = rowsOfCurrentWorker.stream()
										 .filter(r -> r.key().equals(rowKey))
										 .findFirst()
										 .orElseGet(() -> {
											Row newRow = new Row(rowKey);
											rowsOfCurrentWorker.add(newRow);
											return newRow;
										 });
			row.put(column, pair._2());
		}

		for (Map.Entry<String, List<Row>> entry : workerToRowMap.entrySet()) {
			String kvsWorkerAddress = entry.getKey();
			List<Row> rowsOfCurrentWorker = entry.getValue();

			// Serialize all rows into a single byte array
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			try {
				for (Row row : rowsOfCurrentWorker) {
					baos.write(row.toByteArray());
					baos.write(10);  // Append newline character
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
			...
			byte[] response = HTTP.doRequest("PUT", "http://" + kvsWorkerAddress + "/data/batched/" + tableName, serializedByteArray).body();
			String result = new String(response);
			if (!result.equals("OK")) 
				throw new RuntimeException("PUT returned something other than OK: "+result);
		}
	...
	public void putBatchRDD(String tableName, Iterable<String> results) throws IOException {
		if (!haveWorkers)
			downloadWorkers();
		Map<String, List<Row>> workerToRowMap = new HashMap<>();

		for (String result : results) {
			String rowKey = Hasher.hash(UUID.randomUUID().toString().replace("-", ""));
			String kvsWorkerAddress = workers.elementAt(workerIndexForKey(rowKey)).address;
		...
		for (Map.Entry<String, List<Row>> entry : workerToRowMap.entrySet()) {
			...
			// Serialize all rows into a single byte array
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			rowsOfCurrentWorker.forEach(row -> {
				try {
					baos.write(row.toByteArray());
					baos.write(10);  // Append newline character
				} catch (IOException e) {
