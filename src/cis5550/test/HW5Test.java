...
public class HW5Test extends GenericTest {
	...
	void runSetup() throws Exception {
		...
		f = new File("__worker1"+File.separator+xtable);
		if (!f.exists())
			f.mkdir();

		PrintWriter dataOut = new PrintWriter("__worker1"+File.separator+xtable+File.separator+xrow);
		dataOut.print(xrow+" "+xcolumn+" "+xvalue.length()+" "+xvalue+" ");
		dataOut.close();

		PrintWriter cfgOut = new PrintWriter("__worker1"+File.separator+"config");
	...
	{
		File idf = new File("__worker1"+File.separator+"id");
		if (idf.exists())
			idf.delete();

		File d = new File("__worker1");
		d.delete();
	}

	@SuppressWarnings("resource")
	void runTests(Set<String> tests) throws Exception {
		File cfg = new File("__worker1"+File.separator+"config");
		if (!cfg.exists()) {
		...
		if (tests.contains("wdisk")) try {
			...
			File f = new File("__worker1"+File.separator+thetable+File.separator+row);
			if (!f.exists())
		...
		if (tests.contains("rdisk")) try {
			...
			if (!r.body().equals(yvalue))
				testFailed("We wrote a file called __worker1/"+ytable+"/"+yrow+" to disk that contained a row with a single column ("+ycol+") that had value '"+yvalue+"'; then we tried to GET that value. The worker did return a 200 status code to our "+requestStr+", but the value wasn't '"+yvalue+"' as we had expected. Instead, we got:\n\n"+dump(r.body));
			s.close();
		...
		if (tests.contains("delete")) try {
			...
			File f = new File("__worker1"+File.separator+table2+File.separator+row2a);
			if (!f.exists())
			...
			f = new File("__worker1"+File.separator+table2+File.separator+row2b);
			if (!f.exists())
			...
			File idf = new File("__worker1"+File.separator+table2+File.separator+row2a);
			if (idf.exists())
			...
			idf = new File("__worker1"+File.separator+table2+File.separator+row2b);
			if (idf.exists())
